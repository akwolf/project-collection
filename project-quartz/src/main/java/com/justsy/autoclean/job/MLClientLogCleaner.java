 ...
 */
public class MLClientLogCleaner extends AbstractFileCleaner {
	@Override
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		String path = propertiesReader.getValue("ml_client.dir");
...
//		logger.debug("ml_client log file exists : " + f.exists());
		File[] file = f.listFiles(new FilenameFilter() {
			...
			public boolean accept(File dir, String name) {
				...
				// logger.debug("file : " + dir.getAbsolutePath() + "<-->" +
				// name
				// + "<-->" + n);
				if (name.endsWith(".dbs") && n.matches("\\d*")) {
					return true;
				}
				return false;
			}
